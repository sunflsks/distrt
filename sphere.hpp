...
class sphere : public hittable {
   public:
    ...
    bool hit(const ray& r, const interval& t_interval,
        ...
        double discriminant =
            (b_half * b_half -
             a * c);  // b^2 - 4ac, substitute and we get
        ...
        double d_sqrt = std::sqrt(
            discriminant);  // sqrt(discriminant / 4) = (1/2)*sqrt(disriminant)
        double root =
            (b_half - d_sqrt) /
            a;  // (-b - disc) / 2a => 2*b_half - 2(1/2(sqrt(disriminant))) / 2a
